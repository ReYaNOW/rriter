...
### 4. Дерево файлов (`app/file_tree.rs`) и Иконки (`file_icons.rs`)
Дерево файлов построено без использования DOM и рекурсии.
...
* **События:** При клике на папку (в `app/input.rs`), массив `FileNode` перестраивается (папка раскрывается/скрывается, новые элементы вставляются или удаляются).
