            ...
            if state == ElementState::Pressed {
                    ...
                    &self.renderer.as_ref().unwrap().sticky_scroll_rects
                {
                        ...
                        && last_mouse_y <= ry + rh
                    {
                        ...
                        let visual_line = self
                            ...
                            .unwrap_or(phys_line);
                        let line_y =
                            visual_line as f32 * self.renderer.as_ref().unwrap().line_height;
                        let wh = self.window.as_ref().unwrap().inner_size().height as f32;
                        let max_scroll = self
                            .renderer
                            .as_mut()
                    ...
                    && last_mouse_y > wh - 14.0 * s
                {
                    if last_mouse_x > left_pad && last_mouse_x < scrollbar_x {
                        ...
                        let thumb_w =
                            (track_w / (max_x + track_w).max(1.0) * track_w).max(40.0 * s);
            ...
            if self.is_dragging_search {
                ...
                if elapsed > 120 || dy > 10.0 {
                    ...
                    self.scroll_y.target =
                        (scroll_ratio * max_scroll).clamp(0.0, max_scroll).round();
