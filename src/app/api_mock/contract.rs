...
pub fn api_mock_contract_from_state_text(
    ...
    let query = parse_contract_class(text, "Query", &base.query);
    let body = parse_contract_class(text, "Body", &base.body);
    let response = parse_contract_class(text, "Response", &base.response);
...
pub fn api_mock_default_handler_body(contract: &ApiMockPythonContract) -> String {
    const INDENT: &str = "    ";
    if !contract.response.enabled {
        return format!("{INDENT}return Response()");
    }
...
fn parse_contract_class(
    ...
    for line in text.lines() {
        ...
        if let Some(field) = parse_contract_field_line(trimmed, base) {
            fields.push(field);
        }
    }
    if !seen {
...
fn parse_contract_field_line(
    ...
    field.default_value = default_text.and_then(python_default_to_contract_default);
...
pub fn api_mock_python_type(field: &ApiMockContractField) -> String {
    let mut base = if !field.enum_values.is_empty() {
        literal_type(&field.enum_values)
    } else {
        match field.kind {
...
mod tests {
    ...
    #[test]
    fn class_text_uses_annotated_literal_and_defaults() {
        let mut field =
        ...
        assert!(out.contains("Literal[\"new\", \"done\"]"));
