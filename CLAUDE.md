...
1. **Lazy Context Loading (Принцип ленивой загрузки):**
   ...
   - Если задача сложная (рефакторинг, borrow checker), запрашивай файлы точечно через `<request_file>path/to/file.rs</request_file>`.
...
В корне проекта лежит `PROJECT_MAP.md` (v4.5) — это твоя «карта местности».
- **Ownership Tree:** Используй карту, чтобы отслеживать, какой модуль владеет данными (App -> Editor -> Renderer).
- **Logic Deps:** Перед внесением изменений проверь список `Logic Deps` в карте. Если ты меняешь функцию, от которой зависят другие модули, ты обязан проверить и их.
- **Обновление:** Если ты добавил новые `pub fn` или `struct`, напомни мне запустить `make api-tree`, чтобы обновить твою карту.
